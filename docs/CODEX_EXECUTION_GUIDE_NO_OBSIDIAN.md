# Codex 执行 RelayCore Cross-AI-Agent 构建的指导对话模板（无 Obsidian 版）

> 用途：把这份文件交给 Codex CLI，按阶段驱动它实现 RelayCore Cross-AI-Agent Control Plane。  
...
docs/RELAYCORE_CROSS_AGENT_BUILD_NO_OBSIDIAN.md # 完整蓝图，只在拆分阶段读取
...
8. 所有长期设计决策写入 RelayCore；如果 RelayCore MCP 暂不可用，则写入 `docs/decisions/` 作为临时替代。
...
- `docs/RELAYCORE_CROSS_AGENT_BUILD_NO_OBSIDIAN.md`：完整方案。
- `docs/CODEX_EXECUTION_GUIDE_NO_OBSIDIAN.md`：本文件。
...
3. If RelayCore is unavailable, explicitly report it.
...
## 3. Codex MCP 接入 RelayCore

如果 RelayCore 当前已经能启动 MCP Server：
...
如果 `relaycore hook codex ...` 已实现，可创建 `.codex/hooks.json`：
...
1. AGENTS.md
2. docs/RELAYCORE_CROSS_AGENT_BUILD_NO_OBSIDIAN.md
...
如果 RelayCore MCP 已可用：

```text
请将本阶段的重要设计决策写入 RelayCore：

- type: decision
- title: <简短标题>
- content: <为什么这样设计>
- rejected: <被否决方案和原因>
- tags: ["relaycore", "cross-agent", "phase-X"]
```

如果 RelayCore MCP 不可用：
...
```text
用 Codex 稳定、低上下文、低 token、可回滚地完成 RelayCore Cross-AI-Agent Control Plane 的 MVP 构建。
```
