# RelayCore Cross-AI-Agent Control Plane 构建蓝图（无 Obsidian 版）

> 版本：v0.3.0  
> 范围：本蓝图**不考虑 Obsidian 接入**。RelayCore 的核心目标是服务 AI Agent 执行，而不是做人类 Markdown 知识库。  
> 目标：把 RelayCore 从“多 Agent 共享记忆库”升级为“跨 AI Agent 的统一长期记忆、命令总线、事件日志、记忆质量治理、低 token 检索与协同控制框架”。

---

## 0. 定位

RelayCore 的核心不是普通笔记系统，也不是给人阅读的 PKM，而是：
...
1. 跨 AI Agent 共享长期记忆。
2. 让 Agent 默认使用 RelayCore，而不是只使用自己的原生 memory。
...
不要试图让 Claude Code 读取 Codex 的内部上下文，也不要让 Codex 直接读取 Claude Code 的私有会话状态。RelayCore 只统一：
...
├── CLAUDE.md
├── docs/
│   ├── RELAYCORE_CROSS_AGENT_BUILD.md
│   ├── RELAYCORE_RUNTIME_CONTRACT.md
│   ├── RELAYCORE_TOKEN_POLICY.md
│   ├── RELAYCORE_MEMORY_QUALITY.md
│   ├── RELAYCORE_EXTERNAL_REFERENCES.md
│   └── RELAYCORE_SECURITY_MODEL.md
├── .claude/
│   └── skills/
│       └── relaycore-cross-agent/
│           └── SKILL.md
├── .codex/
│   ├── config.toml
│   └── hooks.json
...
```markdown
# RelayCore Runtime Contract

RelayCore is the only long-term memory backend.
...
If RelayCore is unavailable, report:
"RelayCore unavailable, cannot persist long-term memory."
```
...
```text
RelayCore is the only long-term memory backend. Before tasks call memory_begin_task and memory_context. Use command_poll for managed sessions. Append important progress only. Before stopping call memory_commit_task and save durable decisions/rejected. Return summaries by default; request full content only by ID when necessary.
```
...
```bash
claude mcp add --transport stdio relaycore -- relaycore mcp --server http://127.0.0.1:9090
```
...
Use RelayCore as the only long-term memory backend.
...
```bash
codex mcp add relaycore -- relaycore mcp --server http://127.0.0.1:9090
```
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "relaycore hook codex session-start",
            "statusMessage": "Joining RelayCore session"
          }
        ]
      }
    ],
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "relaycore hook codex stop",
            "statusMessage": "Committing RelayCore summary"
          }
        ]
      }
    ]
...
- [ ] Claude Code 和 Codex CLI 均能接入同一 RelayCore MCP。
...
```text
请阅读 docs/RELAYCORE_CROSS_AGENT_BUILD_NO_OBSIDIAN.md，并按 Phase 顺序实现 RelayCore Cross-AI-Agent Control Plane MVP。
...
6. 所有长期决策写入 RelayCore knowledge。
